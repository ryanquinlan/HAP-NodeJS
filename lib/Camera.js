...
Camera.prototype.handleSnapshotRequest = function(request, callback) {
  ...
  imageRequest.get(this.snapshotUrl, function (err, res, body) {
    callback(err, body);
  });
}
...
Camera.prototype.prepareStream = function(request, callback) {
  ...
  console.log("Received prepareStream", request);
  ...
  let currentAddress = ip.address("enp2s0");
  var addressResp = {
